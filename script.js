...
function RandomBauCua() {
  ...
  setTimeout(() => {
    ...
    for (var i = 0; i < ValueBC.length; i++) {
      if (ResultBet.includes(NameBC[i])) Balance += ValueBC[i];
      ValueBC[i] = 0;
      $("#BetNowBC" + i).textContent =
        NameBC[i] + ": " + ConvertMoney(ValueBC[i]) + "$";
    }
  ...
  item.onclick = function () {
    if (ChipNow <= Balance) {
      ...
      item.insertAdjacentHTML(
        "beforeend",
        "<img class='chipBet' style='--randomIndexT:" +
          coordT +
          "px; --randomIndexL:" +
          coordL +
          "px;' src='Chips/Chip" +
      ...
      $("#BetNowBC" + index).textContent =
        NameBC[index] + ": " + ConvertMoney(ValueBC[index]) + "$";
      ChangeBalance();
    } else {
      Swal.fire(
        "Không Đủ Tiền",
